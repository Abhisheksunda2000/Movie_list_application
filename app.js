...
async function fetchMovies(keyword) {
  try {
    ...
    const response = await fetch(
      `${BASE_URL}?s=${keyword}&apikey=${MY_API_KEY}&page=${currentPageNum}`
    );
...
function nextPage() {
  const maxPageNum = Math.ceil(totalMovies / moviesPerPage);
...
function addComment(event, movieId) {
  if (event.key === "Enter") {
    ...
    openMovieDetails(currentMoviesData.find((movie) => movie.imdbID === movieId));
  }
}
